...
macro(GENERATE_UNIX_EXECUTABLE_SCRIPT)
	if (UNIX)
		GENERATE_UNIX_SCRIPT(
			"${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/run_${PROJECT_${CURRENT_PROJECT_NAME}_TARGETNAME}.sh"
			"./${PROJECT_${CURRENT_PROJECT_NAME}_TARGETNAME}"
		)

		GENERATE_UNIX_SCRIPT(
			"${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/gdb_${PROJECT_${CURRENT_PROJECT_NAME}_TARGETNAME}.sh"
			"gdb ./${PROJECT_${CURRENT_PROJECT_NAME}_TARGETNAME}"
		)
		#set (scriptT "\#!/bin/sh\nexport LD_LIBRARY_PATH=")
...
macro(GENERATE_PLUGIN_ARTIFACTS)
	if (WIN32)
		GET_FILENAME_COMPONENT(PLUGINS_DESTINATION_DIR_TMP  "[HKEY_LOCAL_MACHINE\\SOFTWARE\\PJWSTK\\MDE;ApplicationDataPath]" ABSOLUTE CACHE)
		...
		add_custom_command(  TARGET ${PROJECT_${CURRENT_PROJECT_NAME}_TARGETNAME}
							 POST_BUILD 
							 COMMAND mkdir \"${4}\\plugins\" | mkdir \"${PLUGINS_DESTINATION_DIR_NATIVE}\\plugins\\${PROJECT_${CURRENT_PROJECT_NAME}_TARGETNAME}\" | copy \"$(SolutionDir)bin\\$(Configuration)\\$(TargetName)$(TargetExt)\" \"${PLUGINS_DESTINATION_DIR_NATIVE}\\plugins\\${PROJECT_${CURRENT_PROJECT_NAME}_TARGETNAME}\\$(TargetName)$(TargetExt)\" )
